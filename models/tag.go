...
//删除
/*
思路：根据需要被删除的标签的id在标签文章表中查找相关记录，从这些记录中获取到文章id，然后根据文章id在文章表中找到对应的记录，
将标签名替换掉，最后删除标签文章中的相关记录
*/
func (tag *Tag) Delete() error {
	...
	if len(list) > 0 {
		...
		//将文章表中的tags替换为逗号
		orm.NewOrm().Raw("update "+table+"set tags = REPLACE(tags,?,',')"+
			"where id in ("+strings.Join(ids, ",")+")", ","+tag.Name+",").Exec()
		//删除标签文章表中的相关记录
...
func (tag *Tag) MergeTo(to *Tag) {
	...
	if len(list) > 0 {
		...
		orm.NewOrm().Raw("update "+new(Post).TableName()+" set tags=REPLACE(tags,?,?) where id in ("+
